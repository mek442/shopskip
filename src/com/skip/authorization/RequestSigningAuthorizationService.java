...
public class RequestSigningAuthorizationService implements AuthorizationService {
    ...
    @Autowired
    public RequestSigningAuthorizationService(ServiceLocator serviceLocator,
     UserService userService, ApplicationConfig applicationConfig) throws Exception, ServiceInstantiationException {
        this.userService = userService;
        this.userRepository =(IUserSession)serviceLocator.getService(ServiceLocator.USERSESSION_SERVICE);
    ...
    public ExternalUser authorize(AuthorizationRequestContext context) {
        ...
        if (context.getAuthorizationToken() != null && context.getRequestDateString() != null && context.getNonceToken() != null) {
            ...
            if (token.length == 2) {
                ...
                User user;
				try {
					user = userRepository.findUserByUuid(userId);
    ...
    private boolean isAuthorized(User user, AuthorizationRequestContext authorizationRequest, String hashedToken) {
        ...
        Token authorizationToken = user.getToken();
        String userTokenHash = encodeAuthToken(authorizationToken.getAuthtoken(), unEncodedString);
            if (hashedToken.equals(userTokenHash)) {
                return true;
            }
        LOG.error("Hash check failed for hashed token: {} for the following request: {} for user: {}",
                new Object[]{authorizationRequest.getAuthorizationToken(), unEncodedString, user.getUuid()});
        return false;
    }
